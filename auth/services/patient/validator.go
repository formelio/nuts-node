...
func (v validator) verifyVP(vp vc.VerifiablePresentation, validAt *time.Time) (credentialSubject PatientIdentityCredentialSubject, proof vc.JSONWebSignature2020Proof, resultErr error) {
	// #2428: NutsPatientCredential should be valid (signature), but does not need to be trusted.
	...
	if len(vcs) != 1 {
		resultErr = newVerificationError("exactly 1 PatientIdentityCredential is required")
		return
	}
	if len(vp.Proof) != 1 {
	...
	var credentialSubjects []PatientIdentityCredentialSubject
	...
	// #2428: NutsPatientCredential trust is derived from the fact that the issuer has a trusted NutsOrganizationCredential
	...
	if err != nil {
		resultErr = fmt.Errorf("unable to check NutsPatientCredential trust status using NutsOrganizationCredential: %w", err)
		return
	}
	if len(nutsOrgCreds) == 0 {
		resultErr = newVerificationError("NutsPatientCredential rejected, issuer does not have a trusted NutsOrganizationCredential")
		return
	}
...
func validateRequiredAttributes(credentialSubject PatientIdentityCredentialSubject) error {
	...
	if credentialSubject.Member.Member.Type != "Patient" {
		return errors.New("credentialSubject.member.member.type must be \"Patient\"")
	}
	return nil
}

type patientVerificationResult struct {
...
func (s patientVerificationResult) Validity() contract.State {
	return s.Status
}

func (s patientVerificationResult) Reason() string {
	return s.InvalidReason
}

func (s patientVerificationResult) VPType() string {
	return VerifiablePresentationType
}

func (s patientVerificationResult) DisclosedAttribute(key string) string {
	return s.disclosedAttributes[key]
}

func (s patientVerificationResult) ContractAttribute(key string) string {
	return s.contractAttributes[key]
}

func (s patientVerificationResult) DisclosedAttributes() map[string]string {
	return s.disclosedAttributes
}

func (s patientVerificationResult) ContractAttributes() map[string]string {
