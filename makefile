...
gen-api:
	...
	oapi-codegen --config codegen/configs/vcr_oidc4vci_v0.yaml docs/_static/vcr/oidc4vci_v0.yaml | gofmt > vcr/api/oidc4vci/v0/generated.go
...
docker:
	$(if $(strip $(version)),$(true),$(error version is empty, please provide a valid value (should match the github tag)))
	docker build --build-arg GIT_COMMIT=${GIT_COMMIT} --build-arg GIT_BRANCH=${GIT_BRANCH} --build-arg GIT_VERSION=${GIT_VERSION} -t europe-west4-docker.pkg.dev/ivido-development/ivido-${IMAGE_ENV}/nuts-node:$(version) .
	docker push europe-west4-docker.pkg.dev/ivido-development/ivido-${IMAGE_ENV}/nuts-node:$(version)
